...
def load_data(path):
   def load_data(path):
    ...
    try:
        ...
        print(f"Error loading matrix_Response: {e}")

    # Load additional datasets (example: ChIP-seq data)
    try:
        data["ChIP_data"] = np.loadtxt(gzip.open(os.path.join(path, "C:\Users\ranga\Desktop\Sem-3\IBS\Project ISB\deepRAM-master\datasets\Chlp-seq out\processed_SERBP1_H1-hESC_HudsonAlpha_AC.seq.gz")), skiprows=1)
    except Exception as e:
        print(f"Error loading ChIP data: {e}")

    # Load additional motif datasets (CTCF, FOS)
    for motif in ['CTCF', 'FOS']:
        try:
            data[motif] = np.loadtxt(gzip.open(os.path.join(path, f"{motif}.tab.gz")), skiprows=1)
        except Exception as e:
